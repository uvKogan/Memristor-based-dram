...
- **Output:**
  - Timestamps in documented units, with an explicit NVMain conversion `cycle = round(tick_ps x CPUFreq_MHz / 1e6)` (1 ps ticks; at 3000 MHz one cycle is 1/3 ns, so tick 1,000,000 ps is cycle 3,000; an earlier version of this note wrote 1e9, a typo caught by the T3.1 implementer).
...
## 8. SPEC run provenance recovered (2026-09-20)

Looked up at the Lead's explicit request, narrowly: one filtered listing of the benchmark directory names, the
`run/`, `exe/` and `data/refrate/input/` folders of the three built benchmarks, the three tiny input-control
files, and gem5's own `m5out/config.ini` and `stats.txt` left by the March runs. Nothing else in the suite was
read, searched or copied.

- **No SPEC run directories exist.** `runcpu` never set up a run (`run/` holds only `list`), so there is no
  `speccmds.cmd`. Only the rate versions are built: `502.gcc_r`, `505.mcf_r`, `519.lbm_r` (binaries
  `exe/cpugcc_r_base.mytest-m64`, `exe/mcf_r_base.mytest-m64`, `exe/lbm_r_base.mytest-m64`). The speed versions
  (`602`, `605`, `619`) are not built. The book's "SPEC CPU 2017" label should name the rate benchmarks.
- **The March traces were generated by launching gem5 from each benchmark's `data/refrate/input/` folder**
  (this is what `cleanup_traces.sh:3` sweeps). gem5 recorded the exact process in `m5out/config.ini`:
  - gcc: `cmd=.../502.gcc_r/exe/cpugcc_r_base.mytest-m64 gcc-pp.c -O3 -finline-limit=0 -fif-conversion -fif-conversion2`,
    `cwd=.../502.gcc_r/data/refrate/input`, memory range 512 MB (the se.py default), fast-forward 500 M,
    O3 50 M instructions. This is the first of the five reference workloads in the input `control` file.
    `stats.txt`: `finalTick 611039170000`, `simSeconds 0.037232`, `simInsts 50000003`, which matches section 4
    (573.8 ms fast-forward plus 37.2 ms O3) and closes the "inputs and --mem-size were never recorded" gap.
  - mcf: `cmd=.../505.mcf_r/exe/mcf_r_base.mytest-m64 inp.in`, same cwd pattern, memory range 8 GB. **That March
    attempt died almost at once**: `stats.txt` is empty and the MemCtrl log stops at tick 131,185,500 ps
    (0.13 ms). mcf was never traced; the cause is unknown because stdout was not kept.
  - lbm: `m5out/` is empty (swept). The reference arguments are in `lbm.in`:
    `3000 reference.dat 0 0 100_100_130_ldc.of`.
- The 143 MB raw gcc MemCtrl log of the March O3 run still exists at
  `502.gcc_r/data/refrate/input/m5out/gcc_raw_trace_O3.txt` (it was not deleted after all). It is not needed for
  the regeneration and stays where it is.
- All three binaries are dynamically linked PIE executables. gem5 SE ran gcc that way in March, so the host
  loader path works on this machine; it is the first thing to check if mcf dies again.
