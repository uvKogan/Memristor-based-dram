...
To compile:
- Locally: `typst compile --font-path fonts book.typ book.pdf` (needs the four
  `fonts/Carlito-*.ttf` files on the font path; a local `typst` install won't find
  them automatically without `--font-path`).
- Or via typst.app: go to https://typst.app → new empty project → upload
  `book.typ`, the `media/media/*.png` images (keep the folder path
  `media/media/`), and the four `fonts/Carlito-*.ttf` files (typst.app picks
  up uploaded fonts automatically) → open `book.typ`, it compiles to PDF.
