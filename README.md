...
```bash
/path/to/gem5.opt --debug-flags=MemCtrl --debug-file=raw_trace.txt se.py --cmd=benchmark_binary --cpu-type=X86O3CPU --caches --l2cache --fast-forward=500000000 --maxinsts=50000000
```
...
- [ ] **AI Workload Integration**: Implement MLPerf inference traces.strength in read-heavy, dense parallel operations.
- [ ] **Rank Interleaving Analysis**: Investigate custom rank/bank topologies.
